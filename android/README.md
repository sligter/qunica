...
外出远程连接现已支持自有 VPS TCP 中继，保留 Noise 端到端加密，无需证书。部署、配置生成及手机地址变更见 [RELAY.md](RELAY.md)。需要同时更新桌面端与 APK。

最新安装包：[VPS 中继版 ARM64 APK](artifacts/qunica-0.1.1-relay-arm64-debug.apk)，包含前面的手机 UI 与文件下载修复，可覆盖安装保留配对信息。
...
- 自有 VPS 中继通过外置 frp 转发 Noise TCP；未加入托管中继、后台常驻、系统推送或 iOS。
