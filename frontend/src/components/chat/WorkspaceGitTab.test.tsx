...
import type { GroupWorkspaceGitDiff, GroupWorkspaceGitStatus, LLMProviderRead } from '@/types/api'
...
function renderTab(nextStatus: GroupWorkspaceGitStatus = status) {
  ...
  queryClient.setQueryData<LLMProviderRead[]>(['llm-providers'], [
    {
      id: 'provider-1',
      name: 'Default AI',
...
describe('WorkspaceGitTab i18n', () => {
  beforeEach(async () => {
    ...
    localStorage.removeItem('qunica:git:commit-message-provider')
    localStorage.removeItem('qunica:git:commit-message-model')
  })
  afterEach(() => {
    cleanup()
    vi.unstubAllGlobals()
  ...
  it('sends the custom commit prompt to the generator', async () => {
    ...
    fireEvent.click(screen.getByText('Commit message settings'))
  ...
  it('sends the selected provider and model to the generator', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      message: 'Generated by selected AI',
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    }))
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining('/workspace-git/commit-message'),
      expect.objectContaining({
        body: '{"llm_provider_id":"provider-2","model":"selected-model"}',
      }),
    ))
    expect(await screen.findByDisplayValue('Generated by selected AI')).toBeVisible()
  })
