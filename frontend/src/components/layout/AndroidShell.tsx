...
import { androidDesktopAddress, initializeAndroidSession, pairAndroidDesktop, retryAndroidPersistence, useAndroidSession } from '@/lib/androidSession'
...
export function AndroidShell({ children }: { children: ReactNode }) {
  ...
  async function connect(offer: string) {
    ...
    try {
      ...
      setConnectionError(zh ? `连接失败：${String(cause)}。请确认同一 Wi-Fi、桌面已开启手机连接，代理绕过局域网；配对码过期请重新生成。` : `Connection failed: ${String(cause)}. Check Wi-Fi, desktop sharing and LAN proxy bypass. Generate a fresh code if it expired.`)
  ...
  if (!ready || !server || editing) return (
    <main className="app-safe-area flex h-full min-h-0 flex-col overflow-y-auto bg-background px-6 py-8">
      ...
      <div className="mx-auto w-full max-w-md flex-1">
        ...
        <p className="mb-8 text-sm leading-6 text-muted-foreground">{zh ? '手机与电脑连接同一 Wi-Fi。在桌面「设置 → 手机连接」生成二维码，扫码建立加密连接。无需证书。' : 'Join the same Wi-Fi as your computer. Open Settings → Phone connection on desktop and scan its QR code. Encrypted, without certificates.'}</p>
        {!ready ? (
