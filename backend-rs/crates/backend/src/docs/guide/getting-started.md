...
Set up AG Swarmer in four steps: add an LLM provider, add a workspace, create an agent, then start a chat.

## The setup order

Nothing works without a provider, so it comes first. Each later step depends on the one before it.

1. **Add an LLM provider.** This holds the API key AG Swarmer uses to call a model. Without one, no agent can reply. See `providers`.
