...
pub async fn stream(
    ...
    if let Some(cursor) = last_event_id(&headers)? {
        ...
        let events = fetch_replay_events(state.db.pool(), &group_id, &cursor).await?;
...
fn last_event_id(headers: &HeaderMap) -> Result<Option<String>, ApiError> {
...
fn parse_replay_cursor(raw: &str) -> Result<ReplayCursor, ApiError> {
...
fn event_kind_from_wire(raw: &str) -> Result<StreamEventKind, ApiError> {
