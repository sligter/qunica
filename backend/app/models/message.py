...
class Message(Base, UUIDPkMixin):
    ...
    sender_id: Mapped[UUID | None] = mapped_column(GUID(), nullable=True)
    message_type: Mapped[str] = mapped_column(String(50), nullable=False)
    content: Mapped[str | None] = mapped_column(Text, nullable=True)
    content_json: Mapped[dict[str, Any] | None] = mapped_column(JSONData, nullable=True)
    # PRD §10.5 column name; renamed Python attr to avoid shadowing the
    # `references` builtin in nothing-major-but-safe-style.
    refs: Mapped[dict[str, Any] | None] = mapped_column("references", JSONData, nullable=True)
    reply_to_message_id: Mapped[UUID | None] = mapped_column(GUID(), nullable=True)
